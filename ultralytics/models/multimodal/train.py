...
from ultralytics.data import build_dataloader, build_multimodal_dataset
from ultralytics.engine.trainer import BaseTrainer
# from ultralytics.engine.trainer_multimodal import MultimodalBaseTrainer  # TODO: 多模态数据集
from ultralytics.models import multimodal
...
class MultiModalDetectionTrainer(BaseTrainer):
    """
    A class extending the MultimodalBaseTrainer class for training based on a detection model.

    Example:
        ```python
        from ultralytics.models.multimodal import MultiModalDetectionTrainer

        args = dict(model="yolo11n.pt", data="coco8.yaml", epochs=3)
        trainer = MultiModalDetectionTrainer(overrides=args)
        trainer.train()
        ```
    """
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        model = MultiModalDetectionModel(cfg, nc=self.data["nc"], verbose=verbose and RANK == -1)
        if weights:
            model.load(weights)
        return model
    ...
    def get_validator(self):
        ...
        return multimodal.MultiModalDetectionValidator(
