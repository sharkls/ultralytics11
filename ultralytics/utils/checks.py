...
def check_amp(model):
    ...
    try:
        if len(model.args.model.split("DE")) > 1:
            from ultralytics import YOLOFusion
            # assert amp_allclose(YOLOFusion("yolo11n.pt", task="detect"), im)
            assert amp_allclose(YOLOFusion("yolo11n-DEYOLO.yaml", task="detect"), im)
        else:
            from ultralytics import YOLO
