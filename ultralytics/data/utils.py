...
def verify_image_label_multimodal(args):
    """Verify one image-label pair."""
    ...
    try:
        ...
        assert im2.format.lower() in IMG_FORMATS, f"invalid image format {im2.format}. {FORMATS_HELP_MSG}"
        if im2.format.lower() in {"jpg", "jpeg"}:
            with open(im_file2, "rb") as f:
                f.seek(-2, 2)
                if f.read() != b"\xff\xd9":  # corrupt JPEG
                    ImageOps.exif_transpose(Image.open(im_file2)).save(im_file2, "JPEG", subsampling=0, quality=100)
            ...
            if nl := len(lb):
                ...
                if len(i) < nl:  # duplicate row check
                    ...
                    if segments:
                        segments = [segments[x] for x in i]
                    msg = f"{prefix}WARNING ⚠️ {im_file}: {nl - len(i)} duplicate labels removed"
                    msg2 = f"{prefix2}WARNING ⚠️ {im_file2}: {nl - len(i)} duplicate labels removed"
            else:
                ne = 1  # label empty
                lb = np.zeros((0, (5 + nkpt * ndim) if keypoint else 5), dtype=np.float32)
        else:
...
def check_det_dataset_multimodal(dataset, autodownload=True):
    """
    Download, verify, and/or unzip a dataset if not found locally.
    ...
    # Set paths
    data["path"] = path  # download scripts
    for k in "train", "train2", "val", "val2", "test", "minival":
        if data.get(k):  # prepend path
            if isinstance(data[k], str):
                x = (path / data[k]).resolve()
                if not x.exists() and data[k].startswith("../"):
    ...
    if val:
        ...
        if not all(x.exists() for x in val):
            ...
            LOGGER.info(f"Dataset download {s}\n")
    if val2:
        val2 = [Path(x).resolve() for x in (val2 if isinstance(val2, list) else [val2])]  # val path
        if not all(x.exists() for x in val2):
            name = clean_url(dataset)  # dataset name with URL auth stripped
            m = f"\nDataset '{name}' images not found ⚠️, missing path '{[x for x in val2 if not x.exists()][0]}'"
            if s and autodownload:
                LOGGER.warning(m)
            else:
                m += f"\nNote dataset download directory is '{DATASETS_DIR}'. You can update this in '{SETTINGS_FILE}'"
...
def save_dataset_cache_file_multimodal(prefix, prefix2, path, x, version):
    """Save an Ultralytics dataset *.cache dictionary x to path."""
    x["version"] = version  # add cache version
    if is_dir_writeable(path.parent):
        if path.exists():
        ...
        path.with_suffix(".cache.npy").rename(path)  # remove .npy suffix
        LOGGER.info(f"{prefix}New cache created: {path}")
        LOGGER.info(f"{prefix2}New cache created: {path}")
    else:
        LOGGER.warning(f"{prefix}WARNING ⚠️ Cache directory {path.parent} is not writeable, cache not saved.")
        LOGGER.warning(f"{prefix2}WARNING ⚠️ Cache directory {path.parent} is not writeable, cache not saved.")
