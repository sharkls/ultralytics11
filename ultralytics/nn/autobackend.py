...
class AutoBackend(nn.Module):
    ...
    def forward(self, im, augment=False, visualize=False, embed=None):
        ...
        if self.pt or self.nn_module:
            y = self.model(im, augment=augment, visualize=visualize, embed=embed) # TODO: Original
            # y = self.model(im, im, augment=augment, visualize=visualize, embed=embed)
    ...
    def forward_multimodal(self, im, im2, augment=False, visualize=False, embed=None):
        ...
        # OpenVINO
            ...
            if self.inference_mode in {"THROUGHPUT", "CUMULATIVE_THROUGHPUT"}:  # optimized for larger batch-sizes
                ...
                for i in range(n):
                    # Start async inference with userdata=i to specify the position in results list
                    async_queue.start_async(inputs={self.input_name: (im[i : i + 1], im2[i : i + 1])}, userdata=i)  # keep image as BCHW
                async_queue.wait_all()  # wait for all inference requests to complete
                y = np.concatenate([list(r.values())[0] for r in results])

            else:  # inference_mode = "LATENCY", optimized for fastest first result at batch-size 1
                y = list(self.ov_compiled_model(im, im2).values())
    ...
    def warmup_multimodal(self, imgsz=(1, 3, 640, 640)):
        """
        Warm up the model by running one forward pass with a dummy input.
