 ...
 * 返回值：ms UTC时间
 */
int64_t CMultiModalFusionAlg::getTimeStamp()
{
...
std::string CMultiModalFusionAlg::getVersion()
{
    return "AlgLib MultiModalFusionAlg V1.0";
}
 ...
 *   初始化输出路径和日志记录版本信息
 */
CMultiModalFusionAlg::CMultiModalFusionAlg(const std::string& p_strExePath):m_strOutPath(p_strExePath)
{   
    m_strOutPath = p_strExePath;
    LOG(INFO)<<"CMultiModalFusionAlg Version: "<< getVersion() <<std::endl; 
}

/**
 * 析构函数
 * 功能：
 *   用于清理在CMultiModalFusionAlg生命周期中分配的资源
 */
 ...
 *   设置算法参数，初始化算法选择和数据类型
 */
bool CMultiModalFusionAlg::initAlgorithm(CSelfAlgParam* p_pAlgParam, const AlgCallback& p_algCallback, void* p_handle)
{   
    if (!p_pAlgParam){
        LOG(ERROR)<< "CMultiModalFusionAlg::initAlgorithm ---- End >>> Failed : No MultiModalFusion Data."<<std::endl;
        return false;
    }

    m_stSelfMultiModalFusionAlgParam = *p_pAlgParam;
    // 回调函数相关参数
    m_callback = p_algCallback;
    m_handle = p_handle;

    // 算法选择
    std::string s_configPath = m_stSelfMultiModalFusionAlgParam.m_strRootPath+ "Configs/Alg/Alg.yaml";
    m_pMakeAlgorithms = std::make_shared<CMakeMultiModalFusionAlg_strategy>(s_configPath);
    ...
    LOG(INFO)<< "CMultiModalFusionAlg::initAlgorithm ---- finish!"<<std::endl;
    return true;    
}
...
{   
    LOG(INFO)<< "CMultiModalFusionAlg::runAlgorithm ---- start."<<std::endl;

    if (!p_pSrcData){
        LOG(ERROR)<< "CMultiModalFusionAlg::runAlgorithm ---- The MultiModalFusion processing incoming data is empty"<<std::endl;
    ...
    LOG(INFO)<< "CMultiModalFusionAlg::runAlgorithm once status:  over."<<std::endl;
}
