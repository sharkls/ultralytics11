...
void Yolov11Pose::execute() 
{
    ...
    // // 打印第一个目标的详细信息用于调试
    // if (!allFrameResult.vecObjectResult().empty()) {
    //     const auto& firstTarget = allFrameResult.vecObjectResult()[0];
    //     LOG(INFO) << "=== 第一个目标信息 ===";
    //     LOG(INFO) << "类别: " << firstTarget.strClass();
    //     LOG(INFO) << "置信度: " << firstTarget.fVideoConfidence();
    //     LOG(INFO) << "边界框: (" << firstTarget.fTopLeftX() << ", " << firstTarget.fTopLeftY() 
    //               << ") to (" << firstTarget.fBottomRightX() << ", " << firstTarget.fBottomRightY() << ")";
    //     LOG(INFO) << "关键点数量: " << firstTarget.vecKeypoints().size();
        
    //     // 打印前几个关键点的信息
    //     const auto& keypoints = firstTarget.vecKeypoints();
    //     for (size_t i = 0; i < std::min(keypoints.size(), size_t(5)); ++i) {
    //         const auto& kp = keypoints[i];
    //         if (kp.confidence() > 0.1f) {
    //             LOG(INFO) << "  关键点 " << i << ": (" << kp.x() << ", " << kp.y() 
    //                       << ") 置信度: " << kp.confidence();
    //         }
    //     }
    //     LOG(INFO) << "=====================";
    // } else {
    //     LOG(WARNING) << "没有检测到任何目标";
    // }
}
...
void Yolov11Pose::prepareBatchData(size_t batch_start, size_t batch_end)
{
    ...
    for (size_t i = batch_start; i < batch_end; ++i) {
        if (i < m_inputData.images.size()) {
            ...
            LOG(INFO) << "Batch image " << (i - batch_start) << " (global " << i << "): "
                      << "size=" << m_inputData.imageSizes[i].first << "x" << m_inputData.imageSizes[i].second
                      << ", data_size=" << m_inputData.images[i].size();
        }
    }
    ...
    for (size_t i = 0; i < m_batchSizes.size(); ++i) {
        LOG(INFO) << "  Image " << i << ": " << m_batchSizes[i].first << "x" << m_batchSizes[i].second;
    }
}
...
std::vector<float> Yolov11Pose::inference()
{
    ...
    for (const auto& size : m_batchSizes) {
        ...
        LOG(INFO) << "Input image size: " << size.first << "x" << size.second;
    }
    ...
    for (size_t i = 0; i < m_batchInputs.size(); ++i) {
        ...
        LOG(INFO) << "处理图像 " << i << ": 实际大小=" << actual_size 
                  << ", 期望大小=" << actual_size << ", 数据大小=" << image_data.size();
    ...
    // LOG(INFO) << "输出形状: [";
    // for (int i = 0; i < output_dims.nbDims; ++i) {
    //     LOG(INFO) << output_dims.d[i] << (i < output_dims.nbDims - 1 ? ", " : "");
    // }
    // LOG(INFO) << "], 输出大小: " << output_size;
...
{   
    ...
    // if (!output.empty()) {
    //     LOG(INFO) << "=== 原始输出数据调试信息 ===";
    //     for (int b = 0; b < std::min(2, current_batch_size); ++b) {
    //         for (int i = 0; i < std::min(3, num_anchors); ++i) {
    //             LOG(INFO) << "Batch " << b << " Anchor " << i << ":";
    //             // 正确计算索引: [b, feature, anchor] -> b * feature_dim * num_anchors + feature * num_anchors + anchor
    //             LOG(INFO) << "  BBox: (" << output[b * feature_dim * num_anchors + 0 * num_anchors + i] 
    //                       << ", " << output[b * feature_dim * num_anchors + 1 * num_anchors + i]
    //                       << ", " << output[b * feature_dim * num_anchors + 2 * num_anchors + i]
    //                       << ", " << output[b * feature_dim * num_anchors + 3 * num_anchors + i] << ")";
    //             LOG(INFO) << "  Class: " << output[b * feature_dim * num_anchors + 4 * num_anchors + i];
    //             LOG(INFO) << "  Keypoint 0: (" << output[b * feature_dim * num_anchors + 5 * num_anchors + i]
    //                       << ", " << output[b * feature_dim * num_anchors + 6 * num_anchors + i]
    //                       << ", " << output[b * feature_dim * num_anchors + 7 * num_anchors + i] << ")";
    //             LOG(INFO) << "  Keypoint 1: (" << output[b * feature_dim * num_anchors + 8 * num_anchors + i]
    //                       << ", " << output[b * feature_dim * num_anchors + 9 * num_anchors + i]
    //                       << ", " << output[b * feature_dim * num_anchors + 10 * num_anchors + i] << ")";
                
    //             // 打印前20个feature的值来确认数据格式
    //             // LOG(INFO) << "  Raw features 0-19:";
    //             // for (int f = 0; f < 20; ++f) {
    //             //     LOG(INFO) << "    [" << f << "] = " << output[b * feature_dim * num_anchors + f * num_anchors + i];
    //             // }
    //         }
    //     }
    //     LOG(INFO) << "==========================";
    ...
    for (int batch_idx = 0; batch_idx < current_batch_size; ++batch_idx) {
        ...
        // // 新增：分析前几个anchor的原始输出值
        // LOG(INFO) << "=== Image " << batch_idx << " 前5个anchor的原始输出值 ===";
        // for (int i = 0; i < std::min(5, num_anchors); ++i) {
        //     float x_raw = output[batch_start + 0 * num_anchors + i];
        //     float y_raw = output[batch_start + 1 * num_anchors + i];
        //     float w_raw = output[batch_start + 2 * num_anchors + i];
        //     float h_raw = output[batch_start + 3 * num_anchors + i];
        //     float conf_raw = output[batch_start + 4 * num_anchors + i];
            
        //     // 修复：计算实际使用的置信度值（与处理逻辑保持一致）
        //     float conf_used = conf_raw;  // 直接使用模型输出值，因为已经是sigmoid激活后的值
            
        //     LOG(INFO) << "Anchor " << i << ": x=" << x_raw << ", y=" << y_raw 
        //               << ", w=" << w_raw << ", h=" << h_raw 
        //               << ", conf_raw=" << conf_raw << ", conf_used=" << conf_used;
        // }
        // LOG(INFO) << "================================================";
        ...
        for (int i = 0; i < num_anchors; ++i) 
        {
            ...
            if (max_conf < conf_thres_) {
                // if (i < 10) {  // 只打印前10个被过滤的anchor
                //     LOG(INFO) << "Image " << batch_idx << " anchor " << i << ": 置信度 " << max_conf << " 低于阈值 " << conf_thres_ << "，跳过";
                // }
                continue;
            }
            ...
            //     LOG(INFO) << "Image " << batch_idx << " anchor " << i << " 坐标转换:";
            //     LOG(INFO) << "  原始: x=" << x << ", y=" << y << ", w=" << w << ", h=" << h;
            //     LOG(INFO) << "  转换后: x1=" << x1 << ", y1=" << y1 << ", x2=" << x2 << ", y2=" << y2;
            //     LOG(INFO) << "  缩放因子: ratio=" << ratio;
            // }
            ...
            // LOG(INFO) << "Image " << batch_idx << " anchor " << i << ": 有效检测，置信度=" << max_conf 
            //           << ", 边界框=(" << x1 << "," << y1 << "," << x2 << "," << y2 << ")";
        }
        ...
        for (int cls = 0; cls < num_classes_; ++cls) {
            ...
            LOG(INFO) << "Image " << batch_idx << " class " << cls << ": " 
                      << cls_boxes.size() << " detections before NMS";
            
            std::vector<int> keep = nms(cls_boxes, cls_scores);
            
            LOG(INFO) << "Image " << batch_idx << " class " << cls << ": " 
                      << keep.size() << " detections after NMS";
