...
// 注册模块
REGISTER_MODULE("ObjectDetection", Yolov11, Yolov11)

Yolov11::Yolov11(const std::string& exe_path) : IBaseModule(exe_path) 
{
    // 构造函数初始化
...
bool Yolov11::init(void* p_pAlgParam) 
{   
    LOG(INFO) << "Yolov11::init status: start ";
    // 1. 配置参数核验
    if (!p_pAlgParam) return false;
    m_poseConfig = *static_cast<objectdetection::YOLOModelConfig*>(p_pAlgParam);
    ...
    // 初始化TensorRT相关配置
    initTensorRT();
    LOG(INFO) << "Yolov11::init status: success ";
    return true;
}

void Yolov11::initTensorRT() 
{
...
void Yolov11::setInput(void* input) 
{   
...
void* Yolov11::getOutput() {
    return &m_outputResult;
}

void Yolov11::execute() 
{
    // 1、执行推理
...
void Yolov11::rescale_coords(std::vector<float>& coords, bool is_keypoint) 
{
...
std::vector<std::vector<float>> Yolov11::process_keypoints(const std::vector<float>& output, const std::vector<std::vector<float>>& boxes) {
...
std::vector<int> Yolov11::nms(const std::vector<std::vector<float>>& boxes, const std::vector<float>& scores) 
{
