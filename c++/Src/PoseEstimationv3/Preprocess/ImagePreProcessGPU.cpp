...
bool ImagePreProcessGPU::init(void* p_pAlgParam) {
    ...
    // 1. 从配置参数中读取预处理参数
    if (!p_pAlgParam) {
        LOG(ERROR) << "Algorithm parameter is null";
        return false;
    }
    ...
    if (stride_ <= 0) {
        LOG(ERROR) << "stride value error: " << stride_;
        return false;
    }
...
bool ImagePreProcessGPU::initCUDA() {
    ...
    LOG(INFO) << "Found " << deviceCount << " CUDA device(s)";
    
    // 2. 设置CUDA设备
    cuda_status = cudaSetDevice(0);  // 使用第一个GPU
    if (cuda_status != cudaSuccess) {
    ...
    cuda_status = cudaGetDeviceProperties(&deviceProp, 0);
    if (cuda_status != cudaSuccess) {
        LOG(ERROR) << "Failed to get device properties: " << cudaGetErrorString(cuda_status);
        return false;
    }
    
    LOG(INFO) << "Using CUDA device: " << deviceProp.name;
    LOG(INFO) << "Device memory: " << deviceProp.totalGlobalMem / (1024 * 1024) << " MB";
...
bool ImagePreProcessGPU::allocateGPUMemory(size_t max_image_size) {
    ...
    if (error != cudaSuccess) {
        LOG(ERROR) << "CUDA error before memory allocation: " << cudaGetErrorString(error);
        return false;
    }
    ...
    if (!m_gpuOutputBuffer) {
        LOG(ERROR) << "GPU output buffer allocation returned null pointer";
        cudaFree(m_gpuInputBuffer);
        m_gpuInputBuffer = nullptr;
        return false;
    }
    ...
    // 验证分配结果
    if (!m_gpuTempBuffer) {
        LOG(ERROR) << "GPU temp buffer allocation returned null pointer";
        cudaFree(m_gpuInputBuffer);
        cudaFree(m_gpuOutputBuffer);
        m_gpuInputBuffer = nullptr;
        m_gpuOutputBuffer = nullptr;
...
void ImagePreProcessGPU::execute() {
    ...
    if (!m_gpuInputBuffer || !m_gpuOutputBuffer || !m_gpuTempBuffer) {
        LOG(ERROR) << "GPU buffers not properly initialized";
        return;
    }
...
void ImagePreProcessGPU::callNormalizeKernel(void* src, void* dst, int width, int height, 
    ...
    // 验证输入参数
    if (!src || !dst) {
        LOG(ERROR) << "Invalid GPU pointers in normalize kernel";
        return;
    }
    
    if (width <= 0 || height <= 0) {
        LOG(ERROR) << "Invalid dimensions in normalize kernel: " << width << "x" << height;
        return;
    }
...
void ImagePreProcessGPU::callHWCtoCHWKernel(void* src, void* dst, int width, int height, 
    ...
    // 验证输入参数
    if (!src || !dst) {
        LOG(ERROR) << "Invalid GPU pointers in HWC to CHW kernel";
        return;
    }
    
    if (width <= 0 || height <= 0) {
        LOG(ERROR) << "Invalid dimensions in HWC to CHW kernel: " << width << "x" << height;
        return;
    }
...
void ImagePreProcessGPU::callPadImageKernel(void* src, void* dst, int src_width, int src_height, 
    ...
    // 验证输入参数
    if (!src || !dst) {
        LOG(ERROR) << "Invalid GPU pointers in pad kernel";
        return;
    }
    
    if (src_width <= 0 || src_height <= 0 || dst_width <= 0 || dst_height <= 0) {
        LOG(ERROR) << "Invalid dimensions in pad kernel: " << src_width << "x" << src_height 
                   << " -> " << dst_width << "x" << dst_height;
        return;
    }
    
    if (pad_top < 0 || pad_left < 0) {
        LOG(ERROR) << "Invalid padding values: top=" << pad_top << ", left=" << pad_left;
        return;
    }
...
void ImagePreProcessGPU::callBgrToRgbKernel(void* src, void* dst, int width, int height, 
    ...
    // 验证输入参数
    if (!src || !dst) {
        LOG(ERROR) << "Invalid GPU pointers in BGR to RGB kernel";
        return;
    }
    
    if (width <= 0 || height <= 0) {
        LOG(ERROR) << "Invalid dimensions in BGR to RGB kernel: " << width << "x" << height;
        return;
    }
...
bool ImagePreProcessGPU::uploadImageToGPU(const cv::Mat& image, void* gpu_buffer) {
    ...
    // 检查GPU缓冲区大小是否足够
    if (image_size > m_maxGPUBufferSize) {
        LOG(ERROR) << "Image size " << image_size << " exceeds GPU buffer size " << m_maxGPUBufferSize;
        return false;
    }
    ...
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
        LOG(ERROR) << "CUDA error before upload: " << cudaGetErrorString(error);
        return false;
    }
    
    // 上传图像到GPU
    cudaError_t status = cudaMemcpyAsync(gpu_buffer, image.data, image_size,
    ...
    if (status != cudaSuccess) {
        LOG(ERROR) << "Failed to synchronize CUDA stream after upload: " << cudaGetErrorString(status);
        return false;
    }
    
    LOG(INFO) << "Successfully uploaded image " << image.cols << "x" << image.rows 
              << " to GPU, size: " << image_size << " bytes";
    return true;
}
...
bool ImagePreProcessGPU::processBatchImagesGPUInPlace(const std::vector<cv::Mat>& srcImages, int targetWidth, int targetHeight,
    ...
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
        LOG(ERROR) << "CUDA error before batch processing: " << cudaGetErrorString(error);
        return false;
    }
    ...
    for (const auto& srcImage : srcImages) {
        if (srcImage.empty()) {
            LOG(ERROR) << "Empty source image in batch";
            return false;
        }
    ...
    // 验证GPU缓冲区分配结果
    if (!gpuResult.gpu_buffer) {
        LOG(ERROR) << "GPU buffer allocation returned null pointer";
        return false;
    }
...
bool ImagePreProcessGPU::processSingleImageGPUInPlace(const cv::Mat& srcImage, int targetWidth, int targetHeight,
    ...
    cudaError_t error = cudaGetLastError();
    if (error != cudaSuccess) {
        LOG(ERROR) << "CUDA error before single image processing: " << cudaGetErrorString(error);
        return false;
    }
    ...
    if (error != cudaSuccess) {
        LOG(ERROR) << "CUDA error after resize kernel: " << cudaGetErrorString(error);
        return false;
    }
    ...
    error = cudaGetLastError();
    if (error != cudaSuccess) {
        LOG(ERROR) << "CUDA error after BGR to RGB kernel: " << cudaGetErrorString(error);
        return false;
    }
    ...
    error = cudaGetLastError();
    if (error != cudaSuccess) {
        LOG(ERROR) << "CUDA error after normalize kernel: " << cudaGetErrorString(error);
        return false;
    }
    ...
    error = cudaGetLastError();
    if (error != cudaSuccess) {
        LOG(ERROR) << "CUDA error after pad kernel: " << cudaGetErrorString(error);
        return false;
    }
    ...
    error = cudaGetLastError();
    if (error != cudaSuccess) {
        LOG(ERROR) << "CUDA error after HWC to CHW kernel: " << cudaGetErrorString(error);
        return false;
    }
