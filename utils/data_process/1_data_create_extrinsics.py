...
def create_identity_homography_files(input_dir, output_dir, homography_path=None):
    ...
    if homography_path is not None:
        ...
        try:
            ...
            print("\n单应性矩阵信息:")
            print(f"矩阵形状: {homography_matrix.shape}")
            print(f"矩阵类型: {homography_matrix.dtype}")
            print("\n矩阵内容:")
            print(homography_matrix)
            print("\n矩阵元素范围:")
            print(f"最小值: {homography_matrix.min():.6f}")
            print(f"最大值: {homography_matrix.max():.6f}")
            print(f"平均值: {homography_matrix.mean():.6f}")
            print(f"标准差: {homography_matrix.std():.6f}")
