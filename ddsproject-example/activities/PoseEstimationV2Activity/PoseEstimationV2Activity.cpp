#include "PoseEstimationV2Activity.h"

// 启动activity方法二：使用REGISTER_ACTIVITY进行注册，然后通过activity_exec命令将activity启动，可传入参数（-c activity配置文件路径）
// REGISTER_ACTIVITY(PoseEstimationV2Activity)

PoseEstimationV2Activity::PoseEstimationV2Activity()
{
}
...
bool PoseEstimationV2Activity::Init()
{
    // 1. 读取私有配置文件内容
    ...
    reader_ = node_->CreateReader<CAlgResult>(topic_config.camera_merged_topic(),
        std::bind(&PoseEstimationV2Activity::ReadCallbackFunc,
    ...
    pose_estimation_alg_->initAlgorithm(&alg_param_, std::bind(&PoseEstimationV2Activity::GetPoseEstimationResultResponseMessageCallbackFunc, this, std::placeholders::_1, std::placeholders::_2), nullptr);
    return true;
}

// 处理camera_merged_topic中的数据
void PoseEstimationV2Activity::ReadCallbackFunc(const CAlgResult &message,
...
{   
    ...
    if(startTimeStamp_ - count_time_ > 1000)
    {
        ...
        LOG(INFO) << "PoseEstimationActivity FPS =================================: " << count_;
        count_ = 0;
    }
    count_++;
...
// 处理算法返回的感知数据
void PoseEstimationV2Activity::GetPoseEstimationResultResponseMessageCallbackFunc(const CAlgResult& res_message, void* data_handle)
{
    ...
    {   
        LOG(INFO) << "PoseEstimationv2 Result  FrameResult size: " << res_ptr->vecFrameResult().size();
        LOG(INFO) << "PoseEstimationv2 Result size: " << res_ptr->vecFrameResult()[0].vecObjectResult().size();
    }
    else{
        LOG(INFO) << "PoseEstimationv2 Result size: 0";
    }
}


void PoseEstimationV2Activity::Start() 
{
    TINFO << "PoseEstimationV2Activity running";
    std::lock_guard<std::mutex> lock(thread_mutex_);
    if (is_running_) return;
    is_running_ = true;
    message_producer_thread_ = std::make_unique<std::thread>(&PoseEstimationV2Activity::MessageProducerThreadFunc, this);
    message_consumer_thread_ = std::make_unique<std::thread>(&PoseEstimationV2Activity::MessageConsumerThreadFunc, this);
}

// 当收到master节点的PAUSE指令，则执行一些清除工作，比如delete线程
void PoseEstimationV2Activity::PauseClear() 
{
    std::lock_guard<std::mutex> lock(thread_mutex_);
    if (!is_running_) return;
    is_running_ = false;
...
void PoseEstimationV2Activity::MessageProducerThreadFunc()
{
    ...
    {   
        ...
        {   
            LOG(INFO) << "PoseEstimationv2Act Publish Result FrameResult size: " << message->vecFrameResult().size();
            LOG(INFO) << "PoseEstimationv2Act Publish Result size: " << message->vecFrameResult()[0].vecObjectResult().size();
        }
        else{
            LOG(INFO) << "PoseEstimationv2Act Publish Result size: 0";
        }

        LOG(INFO) << "PoseEstimationV2Activity MessageProducerThreadFunc time:----------------------------------- " << endTimeStamp_ - startTimeStamp_;
        LOG(INFO) << "PoseEstimationV2Activity TimeStamp : " << message->lTimeStamp();
        writer_->SendMessage((void*)message.get());
        // std::this_thread::sleep_for(std::chrono::milliseconds(2000));
    }
}

void PoseEstimationV2Activity::MessageConsumerThreadFunc()
{
    while (is_running_.load())
    {      
        // 输入到融合算法的数据
        ...
        // 执行融合算法
        // LOG(INFO) << "PoseEstimationV2Activity Algorithm InputData get !!! ---------- CMultiModalSrcData : " << l_pMultiModalSrcData->vecVideoSrcData().size();
...
{   
    ...
    google::InitGoogleLogging("PoseEstimationV2Activity");

    std::string activity_info_path = "../../../ddsproject-example/activities/conf/PoseEstimationV2Activity.info";
    ActivityInfo activity_info;
    // 解析PoseEstimationV2Activity配置文件
    GetProtoFromFile(activity_info_path, &activity_info);

    // 设置当前进程名，用于在日志中打印
    SetName("PoseEstimationV2Activity");

    PoseEstimationV2Activity *activity = new PoseEstimationV2Activity();
