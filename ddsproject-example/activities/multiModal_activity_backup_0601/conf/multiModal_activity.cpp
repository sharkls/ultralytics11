...
bool ActivityMultiModal::Init()
{
    ...
    XYZ_safeDeque.SetMaxSize(10);
    M3JUVC_safeDeque.SetMaxSize(10);
...
void ActivityMultiModal::ImgProducerThreadFunc()
{
    ...
    while (is_running_.load())
    {
        ...
        {
            int retry_XYZ = 0;
            while(!XYZ_safeDeque.PopFront(img_XYZ, 10))
            {
                // std::this_thread::sleep_for(std::chrono::milliseconds(10));
                if (++retry_XYZ > 100)
                {
                    std::cout << "There has been no data from XYZ for " << retry_XYZ / 100 << " second." << std::endl;
                    // break;
                }
            }
        }
        ...
        {
            int retry_M3JUVC = 0;
            while(!M3JUVC_safeDeque.PopFront(img_M3JUVC, 10))
            {
                // std::this_thread::sleep_for(std::chrono::milliseconds(10));
                if (++retry_M3JUVC > 100)
                {
                    std::cout << "There has been no data from M3JUVC for " << retry_M3JUVC / 100 << " second." << std::endl;
                    // break;
                };
            }
        }
